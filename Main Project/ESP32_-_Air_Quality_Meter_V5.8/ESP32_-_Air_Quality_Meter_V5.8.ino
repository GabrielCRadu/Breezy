...
int LimitaT = 30, LimitaU = 65;               //Valori limita temperatura si umiditate, pentru alarma, pot fi modificate din setari
...
float ConcentratieGaz, VOCmax, VOCmin = 1000;           //Variabile MQ-2
int TimpIncalzireMQ2 = 5 * 60000;                       //Timp de asteptare pana se incalzeste senzorul de gaze volatile (5 minute)
int LimitaVOC = 15;                                     //Valoare limita VOC, pentru alarma, poate fi modificata din setari
...
if(WiFi.status() != WL_CONNECTED  && WifiOn)tft.pushImage(450,0,30,30, NotConnected2Wifi);  

  else if(WifiOn)tft.pushImage(450,0,30,30, Connected2Wifi);
  else  tft.pushImage(450,0,30,30, WifiOff);
...
// HOME //
    if (Menu == 1) {                                                      //Cand meniul 1"HOME" este selectat
      ...
      if (Temperatura>=LimitaT&&SoundOn)tft.setTextColor(TFT_RED, TFT_BLACK);  
        ...
        tft.fillRect(240, 80, 80, 20, TFT_BLACK);                          
        tft.drawFloat(Temperatura, 1, 240, 80, 4);  
        tft.setTextColor(TFT_WHITE, TFT_BLACK);

      if (Umiditate >= LimitaU&&SoundOn)tft.setTextColor(TFT_RED, TFT_BLACK);
        else tft.setTextColor(TFT_WHITE, TFT_BLACK);
  
        tft.drawFloat(Umiditate, 1, 240, 120, 4);    
        tft.setTextColor(TFT_WHITE, TFT_BLACK);

        tft.fillRect(240, 160, 100, 20, TFT_BLACK);
        tft.drawFloat(Presiune, 1, 240, 160, 4);   

      if(ConcentratieGaz>LimitaVOC&&SoundOn)tft.setTextColor(TFT_RED, TFT_BLACK);   
        else tft.setTextColor(TFT_WHITE, TFT_BLACK);

        tft.fillRect(240, 200, 120, 20, TFT_BLACK);  
        tft.drawFloat(ConcentratieGaz, 1, 240, 200, 4);
        //Serial.print(ConcentratieGaz);
        
         
      if ((Temperatura >= LimitaT || Umiditate >= LimitaU || ConcentratieGaz>LimitaVOC) && SoundOn) digitalWrite(buzzer, HIGH);
        else digitalWrite(buzzer, LOW);
    }
    else digitalWrite(buzzer, LOW);       //In caz ca meniul selectat nu este 1"HOME" se pune pinul buzzerului pe low
// HOME //
...
// SETARI //
      if(Menu==4){                                                      //Cand meniul 4"SETARI" este selectat
        tft.setTextColor(TFT_WHITE, TFT_BLACK);                         //Se seteaza initial culoarea textului la alb cu fundal negru
        tft.fillRect(120, 130, 240, 100, TFT_BLACK);                    //Se face refresh la mijlocul ecranului unde sunt afisate valori, nume variabile selectate
        ...
        if(VariabilaSelectata == 5){                                    //Valoarea 5 a VariabilaSelectata corespunde setarii valorii la care alerta de Compusi Organici Volatili se declanseaza
         ...
         if(VariabilaSelectata == 6){                                   //Valoarea 6 a VariabilaSelectata corespunde setarii valorii la care alerta de Compusi Organici Volatili se declanseaza
          tft.textdatum = TC_DATUM;                                     //Se schimba alinierea textului la Top_Centre(se scrie de sus in jos, centrat pe orizontala fata de coordonata data)
          tft.drawString("Wifi", 240, 195, 4);                          //Se scrie pe ecran ce variabila se modifica (Alerta VOC)
...
void ThingSpeakUpload() {
    if(WifiOn){
        ...
        ThingSpeak.setField(4, ConcentratieGaz);
...
//In functie de variabila este selectata la apasarea butonului aceasta se incrementeaza corespunzator tipului de date//
      ...
      if (key[7].justPressed() && Menu == 4) {               //Daca se apasa butonul 7(<) si suntem in meniul 4 "SETARI" se schimba intr-o directie variabila selectata
            ...
            VariabilaSelectata--;   
            if(VariabilaSelectata<1)VariabilaSelectata = 6;
      }
      ...
      if (key[8].justPressed() && Menu == 4) {               //Daca se apasa butonul 8(>) si suntem in meniul 4 "SETARI" se schimba in cealalta directie variabila selectata  
            ...
            VariabilaSelectata++;   
            if(VariabilaSelectata>6)VariabilaSelectata = 1;
      }
  }
...
//Se afiseaza  textul static, deoarece nu se mai actualizeaza cresc performanta//
    ...
    if (BmeUM == 1)tft.drawString("hPa", 360, 160, 4);
    if (BmeUM == 2)tft.drawString("mmHg", 360, 160, 4);
